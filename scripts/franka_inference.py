...
while True:
    # Wait for an observation from robot simulation
    observation = socket.recv_json()
    print("Received observation:", np.array(observation).shape)

    # Simulate NN inference (e.g., by using some deep learning model)
    # Here, we just send back a dummy result
    action = np.random.randn(8) # sample random action
    result = {"action": action.tolist(), "confidence": 0.9}
    
    # Send the inference result back to robot simulation script
